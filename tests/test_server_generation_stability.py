...
class ServerGenerationStabilityTests(unittest.TestCase):
    ...
    def test_detect_greeting_excessive_duration_artifact_flags_too_long_hi_name(self) -> None:
        sr = 24000
        t = np.linspace(0.0, 2.8, int(sr * 2.8), endpoint=False, dtype=np.float32)
        wav = (0.10 * np.sin(2.0 * np.pi * 210.0 * t)).astype(np.float32)

        stats = server_tts.detect_greeting_excessive_duration_artifact("Hi, Dennis.", wav, sr)

        self.assertEqual(stats["checked"], 1)
        self.assertEqual(stats["artifact"], 1)
        self.assertGreaterEqual(stats["duration_ms"], stats["expected_max_ms"])

    def test_detect_greeting_excessive_duration_artifact_allows_normal_hi_name(self) -> None:
        sr = 24000
        t = np.linspace(0.0, 1.4, int(sr * 1.4), endpoint=False, dtype=np.float32)
        wav = (0.10 * np.sin(2.0 * np.pi * 210.0 * t)).astype(np.float32)
